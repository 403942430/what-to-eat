...
> 状态: 已完成
> 说明: Tampermonkey 用户脚本实现淘宝闪购评论自动标记、历史订单抓取，Kiwi ↔ PWA 通过 Railway 云端桥接自动同步。
...
| 状态 | 待办 |
| 依赖 | 无 |
| 描述 | Tampermonkey 脚本：自动翻页抓取淘宝闪购 H5 订单页，提取店名/商品/价格/日期，导出 JSON 到剪贴板 |
| 涉及文件 | `userscript/taobao-flash-review.user.js` |
| 验证命令 | 手动: Kiwi 浏览器安装脚本 → 打开淘宝闪购 H5 订单页 → 点击抓取 → 检查剪贴板是否有 JSON |
| 验证预期 | 粘贴输出的 JSON 包含正确的店铺和订单数据 |
| 重试次数 | 0/3 |
| 完成时间 | - |
| 备注 | 依赖淘宝闪购 H5 网页版存在，需实际验证 DOM 结构 |
...
| 涉及文件 | `userscript/taobao-flash-review.user.js`（追加） |
| 验证命令 | 手动: 打开淘宝闪购某店铺评论区 → 确认评论旁显示标记 |
